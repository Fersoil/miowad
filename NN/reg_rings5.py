...
xor3_test = xor3_test.drop(xor3_test.index)
    ...
    for early_stop in early_stops:
        ...
        for run in range(5):
            ...
            if len(losses) < max_epochs:
                losses = np.pad(losses, (0, max_epochs - len(losses)), 'constant', constant_values=(None, None))
                test_losses = np.pad(test_losses, (0, max_epochs - len(test_losses)), 'constant', constant_values=(None, None))
        ...
        plt.ylabel("Loss")
        plt.legend()
        plt.savefig(f'plots/rings5/{reg}_{early_stop}.png', bbox_inches='tight')
