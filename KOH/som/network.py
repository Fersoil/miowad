...
class KohonenNetwork:
    def __init__(self, M: int, N: int, neighboring_func: Literal["L2", "gaussian", "minus_one_gaussian", "mexican"] = "gaussian", sigma: float=1.0, time_scale: bool=True, grid: Literal["rectangular", "hexagonal"]="rectangular",
                init_method: Literal["uniform", "random"]="random", neighboring_width: float = 1.0, vec_dim: int = 2, lambda_param: float = 10, dataset: np.ndarray = None) -> None:
        ...
        if neighboring_func == "L2":
            self.neighboring_func = DistNeighboringFunc(sigma = sigma, time_scale=time_scale)
        elif neighboring_func == "gaussian":
            self.neighboring_func = GaussianNeighboringFunc(sigma = sigma, time_scale=time_scale)
        elif neighboring_func == "minus_one_gaussian":
            self.neighboring_func = MinusOneGaussianNeighboringFunc(sigma = sigma, time_scale=time_scale)
        elif neighboring_func == "mexican":
            self.neighboring_func = MexicanSombreroNeighboringFunc(sigma = sigma, time_scale=time_scale)    
        ...
        if dataset is not None:
            ...
            self.init_cells(method=init_method, low = np.min(dataset), high = np.max(dataset))
        else:
            self.init_cells(method = init_method)



    def init_cells(self, method: Literal["random", "uniform"], low = 0.0, high = 1.0) -> None:
        if method == "random":
            self.cells = np.random.uniform(low, high, (self.M, self.N, self.vec_dim))
        else:
    ...
    def update_cells(self, x: np.ndarray, i: int, j: int, t: int) -> None:
        ...
        for m in range(self.M):
            for n in range(self.N):
                ...
                self.cells[m, n] += self.neighboring_width * self.alpha(t) * self.neighboring_func(current_unit, best_unit, t) * (x - self.cells[m, n])
